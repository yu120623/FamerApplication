...
public class CalendarView extends LinearLayout implements ScrollableConst, TouchableConst {
    ...
    private Map<Calendar,String> dateAndPrice = new HashMap<>();
    /**
    ...
    private void setDaysInCalendar() {
        ...
        for (int i = 1; i < 43; i++) {
            ...
            for(Calendar key : dateAndPrice.keySet()){
                if(CalendarUtil.isSameDay(key,startCalendar)){
                    dayView.getPriceText().setVisibility(View.VISIBLE);
                    dayView.getPriceText().setText(dateAndPrice.get(key));
                    break;
                }else{
                    dayView.getPriceText().setVisibility(View.INVISIBLE);
                }
            }
            if (null != getTypeface()) {
                dayView.getDayText().setTypeface(getTypeface());
            }

            if (CalendarUtil.isSameMonth(calendar, startCalendar)) {
                dayOfMonthContainer.setOnClickListener(onDayOfMonthClickListener);
                dayView.setBackgroundColor(mCalendarBackgroundColor);
                dayView.getDayText().setTextColor(mDayOfWeekTextColor);
            } else {
                dayView.setBackgroundColor(mDisabledDayBackgroundColor);
                dayView.getDayText().setTextColor(mDisabledDayTextColor);
...
//
//            dayOfMonth.getDayText().setTextColor(mCurrentDayOfMonth);
...
//        }
    ...
    private OnClickListener onDayOfMonthClickListener = new OnClickListener() {
        ...
        public void onClick(View view) {
            ...
            final DayView dayOfMonthText = (DayView) view.findViewWithTag(mContext.getString(R.string.day_of_month_text) + tagId);

            // Fire event
            final Calendar calendar = Calendar.getInstance();
            calendar.setFirstDayOfWeek(mFirstDayOfWeek);
            calendar.setTime(mCalendar.getTime());
            calendar.set(Calendar.DAY_OF_MONTH, Integer.valueOf(dayOfMonthText.getDayText().getText().toString()));
